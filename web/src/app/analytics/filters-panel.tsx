...
import { METRIC_LABELS, type MetricLabel } from '@/lib/analytics/metrics';
import { type TierLabel } from '@/lib/analytics/tiering';
import type { FilterState } from '@/lib/analytics/filters';
...
function countActiveFilters(state: FilterState, defaults: FilterState): number {
  let c = 0;
  ...
  if (state.leagues.length) c += 1;
  if (state.teams.length) c += 1;
  return c;
}
        ...
        <Field label="Metric">
          <Dropdown
            ...
            onOptionSelect={(_, data) => {
              if (data.optionValue) update('metric', data.optionValue as MetricLabel);
            }}
          >
            {METRIC_LABELS.map((m) => (
